...
def recover_mix(x, patch_h, patch_w, bbx1, bbx2, bby1, bby2):
...
class AttBlock(nn.Module):
    def __init__(self, norm_layer=nn.LayerNorm, dim=512, num_heads=8, mlp_ratio=2):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.mlp_hidden_dim = int(dim * mlp_ratio)
        self.attn = NystromAttention(dim=dim, dim_head=dim // 8, heads=num_heads, num_landmarks=dim // 2, pinv_iterations=6,
            residual=True, dropout=0.1)
        self.mlp = Mlp(in_features=dim, hidden_features=self.mlp_hidden_dim, act_layer=nn.GELU, drop=0.1)
